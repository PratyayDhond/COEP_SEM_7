...
int function(const char *name, DataType returnType, int paramCount, DataType paramTypes[]) {
...
void removeCarriageReturn(char* input){
    char* p = input;
    while (*p != '\0'){
        if (*p == '\n'){
            *p = '\0';
...
void processInput(char *input) {
    ...
    if (strcmp(command, "var") == 0) {
        ...
        if (returnDataType != UNDEFINED) {
            function(funcName, returnDataType, paramCount, paramTypes);
        } else {
...
int main() {
    ...
    printf("Enter commands (var, func, assign), or Ctrl + C to exit.\n"); //divide call has been skipped here
